/*
 * Copyright 2026 HM Revenue & Customs
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package uk.gov.hmrc.automatedexportsystemfrontend.forms.mappings

import generators.Generators
import org.scalacheck.Gen
import org.scalatestplus.scalacheck.ScalaCheckPropertyChecks
import play.api.data.validation.{Invalid, Valid}
import uk.gov.hmrc.automatedexportsystemfrontend.helpers.SpecBase
import uk.gov.hmrc.automatedexportsystemfrontend.config.CurrencyFormatter
import uk.gov.hmrc.automatedexportsystemfrontend.forms.mappings.Constraints

import java.time.LocalDate

class ConstraintsSpec extends SpecBase with ScalaCheckPropertyChecks with Generators with Constraints {

  "firstError" - {

    "must return Valid when all constraints pass" - {
      val result = firstError(maxLength(10, "error.length"), regexp("""^\w+$""", "error.regexp"))("foo")
      result shouldBe Valid
    }

    "must return Invalid when the first constraint fails" - {
      val result = firstError(maxLength(10, "error.length"), regexp("""^\w+$""", "error.regexp"))("a" * 11)
      result shouldBe Invalid("error.length", 10)
    }

    "must return Invalid when the second constraint fails" - {
      val result = firstError(maxLength(10, "error.length"), regexp("""^\w+$""", "error.regexp"))("")
      result shouldBe Invalid("error.regexp", """^\w+$""")
    }

    "must return Invalid for the first error when both constraints fail" - {
      val result = firstError(maxLength(-1, "error.length"), regexp("""^\w+$""", "error.regexp"))("")
      result shouldBe Invalid("error.length", -1)
    }
  }

  "minimumValue" - {

    "must return Valid for a number greater than the threshold" - {
      val result = minimumValue(1, "error.min").apply(2)
      result shouldBe Valid
    }

    "must return Valid for a number equal to the threshold" - {
      val result = minimumValue(1, "error.min").apply(1)
      result shouldBe Valid
    }

    "must return Invalid for a number below the threshold" - {
      val result = minimumValue(1, "error.min").apply(0)
      result shouldBe Invalid("error.min", 1)
    }
  }

  "maximumValue" - {

    "must return Valid for a number less than the threshold" - {
      val result = maximumValue(1, "error.max").apply(0)
      result shouldBe Valid
    }

    "must return Valid for a number equal to the threshold" - {
      val result = maximumValue(1, "error.max").apply(1)
      result shouldBe Valid
    }

    "must return Invalid for a number above the threshold" - {
      val result = maximumValue(1, "error.max").apply(2)
      result shouldBe Invalid("error.max", 1)
    }
  }

  "regexp" - {

    "must return Valid for an input that matches the expression" - {
      val result = regexp("""^\w+$""", "error.invalid")("foo")
      result shouldBe Valid
    }

    "must return Invalid for an input that does not match the expression" - {
      val result = regexp("""^\d+$""", "error.invalid")("foo")
      result shouldBe Invalid("error.invalid", """^\d+$""")
    }
  }

  "maxLength" - {

    "must return Valid for a string shorter than the allowed length" - {
      val result = maxLength(10, "error.length")("a" * 9)
      result shouldBe Valid
    }

    "must return Valid for an empty string" - {
      val result = maxLength(10, "error.length")("")
      result shouldBe Valid
    }

    "must return Valid for a string equal to the allowed length" - {
      val result = maxLength(10, "error.length")("a" * 10)
      result shouldBe Valid
    }

    "must return Invalid for a string longer than the allowed length" - {
      val result = maxLength(10, "error.length")("a" * 11)
      result shouldBe Invalid("error.length", 10)
    }
  }

  "maxDate" - {

    "must return Valid for a date before or equal to the maximum" - {

      val gen: Gen[(LocalDate, LocalDate)] = for {
        max <- datesBetween(LocalDate.of(2000, 1, 1), LocalDate.of(3000, 1, 1))
        date <- datesBetween(LocalDate.of(2000, 1, 1), max)
      } yield (max, date)

      forAll(gen) { case (max, date) =>
        val result = maxDate(max, "error.future")(date)
        result shouldBe Valid
      }
    }

    "must return Invalid for a date after the maximum" - {

      val gen: Gen[(LocalDate, LocalDate)] = for {
        max <- datesBetween(LocalDate.of(2000, 1, 1), LocalDate.of(3000, 1, 1))
        date <- datesBetween(max.plusDays(1), LocalDate.of(3000, 1, 2))
      } yield (max, date)

      forAll(gen) { case (max, date) =>
        val result = maxDate(max, "error.future", "foo")(date)
        result shouldBe Invalid("error.future", "foo")
      }
    }
  }

  "minDate" - {

    "must return Valid for a date after or equal to the minimum" - {

      val gen: Gen[(LocalDate, LocalDate)] = for {
        min <- datesBetween(LocalDate.of(2000, 1, 1), LocalDate.of(3000, 1, 1))
        date <- datesBetween(min, LocalDate.of(3000, 1, 1))
      } yield (min, date)

      forAll(gen) { case (min, date) =>
        val result = minDate(min, "error.past", "foo")(date)
        result shouldBe Valid
      }
    }

    "must return Invalid for a date before the minimum" - {

      val gen: Gen[(LocalDate, LocalDate)] = for {
        min <- datesBetween(LocalDate.of(2000, 1, 2), LocalDate.of(3000, 1, 1))
        date <- datesBetween(LocalDate.of(2000, 1, 1), min.minusDays(1))
      } yield (min, date)

      forAll(gen) { case (min, date) =>
        val result = minDate(min, "error.past", "foo")(date)
        result shouldBe Invalid("error.past", "foo")
      }
    }
  }

  "minimumCurrency" - {

    "must return Valid for a number greater than the threshold" - {
      val result = minimumCurrency(1, "error.min").apply(BigDecimal(1.01))
      result shouldBe Valid
    }

    "must return Valid for a number equal to the threshold" - {
      val result = minimumCurrency(1, "error.min").apply(1)
      result shouldBe Valid
    }

    "must return Invalid for a number below the threshold" - {
      val result = minimumCurrency(1, "error.min").apply(0.99)
      result shouldBe Invalid("error.min", CurrencyFormatter.currencyFormat(1))
    }
  }

  "maximumCurrency" - {

    "must return Valid for a number less than the threshold" - {
      val result = maximumCurrency(1, "error.max").apply(0)
      result shouldBe Valid
    }

    "must return Valid for a number equal to the threshold" - {
      val result = maximumCurrency(1, "error.max").apply(1)
      result shouldBe Valid
    }

    "must return Invalid for a number above the threshold" - {
      val result = maximumCurrency(1, "error.max").apply(1.01)
      result shouldBe Invalid("error.max", CurrencyFormatter.currencyFormat(1))
    }
  }
}
