...
class MappingsSpec extends SpecBase with OptionValues with Mappings:
  ...
  "currency" - {
    ...
    "should not bind a number with more than 2 decimal places" in {
      val result = testForm.bind(Map("value" -> "1.234"))
      result.errors should contain only FormError("value", "error.invalidNumeric")
    }

    "should not bind negative numbers" in {
      val result = testForm.bind(Map("value" -> "-1"))
      result.errors should contain only FormError("value", "error.nonNumeric")
    }
    ...
    "must unbind a valid value" in {
      ...
      result.apply("value").value.value shouldBe "1"
    }
  }
