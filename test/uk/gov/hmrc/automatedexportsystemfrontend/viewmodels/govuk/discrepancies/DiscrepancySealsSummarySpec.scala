...
import uk.gov.hmrc.automatedexportsystemfrontend.viewmodels.checkAnswers.discrepancies.DiscrepancySealsSummary
import uk.gov.hmrc.automatedexportsystemfrontend.viewmodels.govuk.all.*

class DiscrepancySealsSummarySpec extends AnyFreeSpec with Matchers {
      ...
      DiscrepancySealsSummary.row(userAnswers) shouldBe Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              uk.gov.hmrc.automatedexportsystemfrontend.controllers.discrepancies.routes.DiscrepancySealsController.onPageLoad(CheckMode).url
            )
