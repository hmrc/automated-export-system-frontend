...
class RichJsValueSpec extends SpecBase with ScalaCheckPropertyChecks with ModelGenerators {
  ...
  "set" - {
    ...
    "must set a value on a JsObject" in {
      ...
      forAll(gen) { case (originalKey, originalValue, pathKey, newValue) =>
        ...
        value.set(path, JsString(newValue)) shouldBe JsSuccess(Json.obj(originalKey -> originalValue, pathKey -> newValue))
      }
    }
    ...
    "must set a nested value on a JsObject" in {
      ...
      value.set(path, JsString("baz")).asOpt.value shouldBe Json.obj("foo" -> Json.obj("bar" -> "baz"))
    }
    ...
    "must set into an array which does not exist" in {
      ...
      value.set(path, JsString("bar")) shouldBe JsSuccess(Json.obj("foo" -> Json.arr("bar")))
    }
    ...
    "must set into an object which does not exist" in {
      ...
      value.set(path, JsString("baz")) shouldBe JsSuccess(Json.obj("foo" -> Json.obj("bar" -> "baz")))
    }
    ...
    "must set nested objects and arrays" in {
      ...
      value.set(path, JsString("baz")) shouldBe JsSuccess(Json.obj("foo" -> Json.arr(Json.obj("bar" -> Json.arr("baz")))))
    }
  }
  ...
  "remove" - {
    ...
    "remove a value from one of many arrays" in {
      ...
      input.remove(path) shouldBe JsSuccess(Json.obj("key" -> JsArray(Seq(Json.toJson(2))), "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2)))))
    }
  }
  ...
  "remove the value if the last value is deleted from an array" in {
    ...
    input.remove(path) shouldBe JsSuccess(Json.obj("key" -> JsArray(), "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2)))))
  }
}
