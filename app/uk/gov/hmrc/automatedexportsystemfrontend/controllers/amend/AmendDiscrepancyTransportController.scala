...
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.amend.AmendDiscrepancyTransportView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class AmendDiscrepancyTransportController @Inject() (
  ...
  formProvider: AmendDiscrepancyTransportFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmendDiscrepancyTransportView
  ...
  def onPageLoad(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(AmendDiscrepancyTransportPage(submissionId)) match {
      case None        => form
  ...
  def onSubmit(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendDiscrepancyTransportPage(submissionId), value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(amendNavigator.nextPage(AmendDiscrepancyTransportPage(submissionId), mode, updatedAnswers))
      )
  }
}
