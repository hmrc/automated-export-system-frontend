...
class AmendDiscrepancySealsController @Inject() (
  ...
  formProvider: AmendDiscrepancySealsFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmendDiscrepancySealsView
  ...
  def onPageLoad(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(AmendDiscrepancySealsPage(submissionId)) match {
      case None        => form
  ...
  def onSubmit(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendDiscrepancySealsPage(submissionId), value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(amendNavigator.nextPage(AmendDiscrepancySealsPage(submissionId), mode, updatedAnswers))
      )
  }
}
