...
class AmendLocationIdController @Inject() (
  ...
  formProvider: AmendLocationIdFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmendLocationIdView
  ...
  def onPageLoad(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(AmendLocationIdPage(submissionId)) match {
      case None        => form
  ...
  def onSubmit(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendLocationIdPage(submissionId), value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(amendNavigator.nextPage(AmendLocationIdPage(submissionId), mode, updatedAnswers))
      )
  }
}
