...
class AmendLocationTypeController @Inject() (
  ...
  formProvider: AmendLocationTypeFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmendLocationTypeView
  ...
  def onPageLoad(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(AmendLocationTypePage(submissionId)) match {
      case None        => form
  ...
  def onSubmit(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendLocationTypePage(submissionId), value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(amendNavigator.nextPage(AmendLocationTypePage(submissionId), mode, updatedAnswers))
      )
  }
}
