...
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.amend.AmendDiscrepancyTransportMeansView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class AmendDiscrepancyTransportMeansController @Inject() (
  ...
  formProvider: AmendDiscrepancyTransportMeansFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmendDiscrepancyTransportMeansView
  ...
  def onPageLoad(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(AmendDiscrepancyTransportMeansPage(submissionId)) match {
      case None        => form
  ...
  def onSubmit(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendDiscrepancyTransportMeansPage(submissionId), value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(amendNavigator.nextPage(AmendDiscrepancyTransportMeansPage(submissionId), mode, updatedAnswers))
      )
  }
}
