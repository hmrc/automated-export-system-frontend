...
class AmendDiscrepancyReferenceController @Inject() (
  ...
  formProvider: AmendDiscrepancyReferenceFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmendDiscrepancyReferenceView
  ...
  def onPageLoad(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(AmendDiscrepancyReferencePage(submissionId)) match {
      case None        => form
  ...
  def onSubmit(mode: Mode, submissionId: String): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(AmendDiscrepancyReferencePage(submissionId), value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(amendNavigator.nextPage(AmendDiscrepancyReferencePage(submissionId), mode, updatedAnswers))
      )
  }
}
