...
import uk.gov.hmrc.automatedexportsystemfrontend.forms.unhappyPath.DiscrepancyTransportDocFormProvider
...
import uk.gov.hmrc.automatedexportsystemfrontend.pages.unhappyPath.DiscrepancyTransportDocPage
import uk.gov.hmrc.automatedexportsystemfrontend.repositories.SessionRepository
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.unhappyPath.DiscrepancyTransportDocView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class DiscrepancyTransportDocController @Inject() (
  ...
  formProvider: DiscrepancyTransportDocFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: DiscrepancyTransportDocView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(DiscrepancyTransportDocPage) match {
      case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(DiscrepancyTransportDocPage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(unhappyPathNavigator.nextPage(DiscrepancyTransportDocPage, mode, updatedAnswers))
      )
  }
}
