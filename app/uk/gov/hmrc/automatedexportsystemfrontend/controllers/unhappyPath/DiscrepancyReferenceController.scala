...
import uk.gov.hmrc.automatedexportsystemfrontend.forms.unhappyPath.DiscrepancyReferenceFormProvider
...
import uk.gov.hmrc.automatedexportsystemfrontend.pages.unhappyPath.DiscrepancyReferencePage
import uk.gov.hmrc.automatedexportsystemfrontend.repositories.SessionRepository
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.unhappyPath.DiscrepancyReferenceView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class DiscrepancyReferenceController @Inject() (
  ...
  formProvider: DiscrepancyReferenceFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: DiscrepancyReferenceView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(DiscrepancyReferencePage) match {
      case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(DiscrepancyReferencePage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(unhappyPathNavigator.nextPage(DiscrepancyReferencePage, mode, updatedAnswers))
      )
  }
}
