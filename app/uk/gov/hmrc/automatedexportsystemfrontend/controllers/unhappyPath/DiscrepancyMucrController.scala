...
class DiscrepancyMucrController @Inject() (
  ...
  formProvider: DiscrepancyMucrFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: DiscrepancyMucrView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(DiscrepancyMucrPage) match {
      case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(DiscrepancyMucrPage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(unhappyPathNavigator.nextPage(DiscrepancyMucrPage, mode, updatedAnswers))
      )
  }
}
