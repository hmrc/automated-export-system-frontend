...
import uk.gov.hmrc.automatedexportsystemfrontend.forms.discrepancies.DiscrepancyTransportFormProvider
import uk.gov.hmrc.automatedexportsystemfrontend.models.{ContainerDetails, Mode}
...
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.discrepancies.DiscrepancyTransportView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class DiscrepancyTransportController @Inject() (
  ...
  formProvider: DiscrepancyTransportFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: DiscrepancyTransportView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(DiscrepancyTransportPage) match {
      case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(DiscrepancyTransportPage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(unhappyPathNavigator.nextPage(DiscrepancyTransportPage, mode, updatedAnswers))
      )
  }
}
