...
import uk.gov.hmrc.automatedexportsystemfrontend.models.{LocationDetails, Mode}
...
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.discrepancies.LocationIdView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class LocationIdController @Inject() (
  ...
  formProvider: LocationIdFormProvider,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(LocationIdPage) match {
      case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(LocationIdPage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(unhappyPathNavigator.nextPage(LocationIdPage, mode, updatedAnswers))
      )
  }
}
