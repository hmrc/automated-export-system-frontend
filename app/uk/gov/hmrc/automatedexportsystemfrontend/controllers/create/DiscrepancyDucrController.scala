...
import uk.gov.hmrc.automatedexportsystemfrontend.forms.create.DiscrepancyDucrFormProvider
...
import uk.gov.hmrc.automatedexportsystemfrontend.pages.create.DiscrepancyDucrPage
import uk.gov.hmrc.automatedexportsystemfrontend.repositories.SessionRepository
import uk.gov.hmrc.automatedexportsystemfrontend.views.html.create.DiscrepancyDucrView
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class DiscrepancyDucrController @Inject() (
  ...
  formProvider: DiscrepancyDucrFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: DiscrepancyDucrView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData) { implicit request =>
    ...
    val preparedForm = request.userAnswers.get(DiscrepancyDucrPage) match {
      case None        => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (actionBuilder andThen getData andThen requireData).async { implicit request =>
    form
      ...
      .fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(DiscrepancyDucrPage, value))
            _ <- sessionRepository.set(updatedAnswers)
          } yield Redirect(createNavigator.nextPage(DiscrepancyDucrPage, mode, updatedAnswers))
      )
  }
}
