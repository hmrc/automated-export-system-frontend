...
package uk.gov.hmrc.automatedexportsystemfrontend.controllers.auth
...
import uk.gov.hmrc.automatedexportsystemfrontend.controllers.auth.routes as authRoute
...
class AuthController @Inject() (
  ...
  def signOutNoSurvey(): Action[AnyContent] = authActionBuilder.async { implicit request =>
    sessionRepository
      .clear(request.sessionId)
      .map { _ =>
        Redirect(config.signOutUrl, Map("continue" -> Seq(authRoute.SignedOutController.onPageLoad().url)))
      }
  }
}
