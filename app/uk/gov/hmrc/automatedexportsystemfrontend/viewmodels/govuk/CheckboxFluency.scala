...
package uk.gov.hmrc.automatedexportsystemfrontend.viewmodels.govuk

import play.api.data.Form
import play.api.i18n.Messages
import play.twirl.api.Html
import uk.gov.hmrc.automatedexportsystemfrontend.viewmodels.ErrorMessageAwareness
...
trait CheckboxFluency {
  ...
  object CheckboxesViewModel extends ErrorMessageAwareness with FieldsetFluency {
    ...
    def apply(form: Form[_], name: String, items: Seq[CheckboxItem], legend: Legend)(implicit messages: Messages): Checkboxes =
      apply(form = form, name = name, items = items, fieldset = FieldsetViewModel(legend))

    def apply(form: Form[_], name: String, items: Seq[CheckboxItem], fieldset: Fieldset)(implicit messages: Messages): Checkboxes =
      Checkboxes(
